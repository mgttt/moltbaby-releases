...
#### 1.6 策略级盈亏口径（uPnL）治理 🔥 **P1 - 新增 (2026-02-19)**
**背景**：当前 `uPnL` 为账户级 symbol 总未实现盈亏，尚未拆分到策略级；在分策略汇报中展示会造成误读。

**决策（总裁指示）**：
- [ ] 在策略级 `uPnL` 真实可用前，**分策略输出不显示浮盈/uPnL**
- [ ] 仅在账户层展示 `account uPnL / lev / netEq`
- [ ] 路线图纳入“策略级 uPnL 真实归因”后再恢复分策略浮盈展示

**实现任务**：
- [ ] 交易所 execution/position 按策略前缀归因（orderLinkId）
- [ ] 建立策略级已实现/未实现盈亏计算链路（含手续费）
- [ ] 冷启动/重启后策略级PnL可恢复（与账本同源）
- [ ] 输出字段区分：`strategyUpnl` vs `accountUpnl`

**验收标准**：
- [ ] 分策略 `uPnL` 与策略账本持仓方向一致，误差可解释
- [ ] 24h 内无“策略级/账户级盈亏口径混报”
- [ ] 汇报模板固定：策略层不混入账户级uPnL
