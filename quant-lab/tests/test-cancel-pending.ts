...
const logger = createLogger('TEST_CANCEL_PENDING');
/**
...
async function test() {
  logger.info('========================================');
  logger.info('   P0 回归测试：cancelOrder pending 检测');
  logger.info('========================================\n');
  ...
  // ============================================================
  logger.info('[测试 1] 撤销 pending 订单（应跳过撤单）');
  ...
  try {
    ...
    if (requestCalls.length === 0) {
      logger.info('✅ 通过：pending 订单未发送 API 请求\n');
      passed++;
    } else {
      logger.info('❌ 失败：pending 订单不应发送 API 请求\n');
      logger.info('API 调用:', requestCalls);
      failed++;
    }
  } catch (error: any) {
    logger.info('❌ 失败：pending 订单不应抛出异常\n');
  ...
  // 测试 2: 撤销真实订单（正确格式）
  // ============================================================
  logger.info('[测试 2] 撤销真实订单（MYXUSDT:12345678）');
  ...
  try {
    ...
    if (requestCalls.length === 1) {
      ...
      if (call.method === 'POST' && 
        ...
        logger.info('✅ 通过：真实订单正确发送 API 请求\n');
        passed++;
      } else {
        logger.info('❌ 失败：API 请求参数不正确\n');
        logger.info('API 调用:', call);
        failed++;
      }
    } else {
      logger.info('❌ 失败：应该发送 1 次 API 请求\n');
      logger.info('实际调用次数:', requestCalls.length);
      failed++;
    }
  } catch (error: any) {
    logger.info('❌ 失败：真实订单不应抛出异常\n');
    logger.info('错误:', error.message);
    failed++;
  }
  ...
  try {
    ...
    logger.info('❌ 失败：错误格式订单应抛出异常\n');
    failed++;
  } catch (error: any) {
    if (error.message.includes('Invalid orderId format')) {
      logger.info('✅ 通过：错误格式订单抛出正确异常\n');
      passed++;
    } else {
      logger.info('❌ 失败：异常信息不正确\n');
      logger.info('错误:', error.message);
      failed++;
    }
  }
  ...
  // ============================================================
  logger.info('[测试 4] 撤销错误格式订单（symbol 为空）');
  ...
  try {
    ...
    logger.info('❌ 失败：空 symbol 应抛出异常\n');
    failed++;
  } catch (error: any) {
    if (error.message.includes('missing symbol or id')) {
      logger.info('✅ 通过：空 symbol 抛出正确异常\n');
      passed++;
    } else {
      logger.info('❌ 失败：异常信息不正确\n');
      logger.info('错误:', error.message);
  ...
  // ============================================================
  logger.info('[测试 5] 撤销错误格式订单（orderId 为空）');
  ...
  try {
    ...
    logger.info('❌ 失败：空 orderId 应抛出异常\n');
    failed++;
  } catch (error: any) {
    if (error.message.includes('missing symbol or id')) {
      logger.info('✅ 通过：空 orderId 抛出正确异常\n');
      passed++;
    } else {
      logger.info('❌ 失败：异常信息不正确\n');
      logger.info('错误:', error.message);
  ...
  logger.info('   测试结果');
  logger.info('========================================');
  logger.info(`通过: ${passed}/5`);
  logger.info(`失败: ${failed}/5`);
