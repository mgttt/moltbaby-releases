...
export class ModuleReloader {
  ...
  async reloadModule(options: ModuleReloadOptions): Promise<ModuleReloadResult> {
    ...
    logger.info(`[ModuleReloader] 开始模块热更新: ${opts.modulePath}`);
    ...
    try {
      ...
      try {
        ...
        logger.info(`[ModuleReloader] 旧模块已加载`);
      } catch (error) {
        logger.warn(`[ModuleReloader] 旧模块不存在，这是首次加载`);
      }
      ...
      if (opts.cacheBust) {
        ...
        logger.info(`[ModuleReloader] 使用cache busting: ${importPath}`);
      }

      // 3. 动态加载新模块
      const newModule = await import(importPath);
      logger.info(`[ModuleReloader] 新模块已加载`);
      ...
      logger.info(`[ModuleReloader] 模块热更新完成 ✅`);
      ...
      logger.error(`[ModuleReloader] 模块热更新失败:`, error);
  ...
  async replaceModuleInstance<T>(
    ...
    logger.info(`[ModuleReloader] 创建新实例: ${NewClass.name}`);
  ...
  async reloadModules(modules: ModuleReloadOptions[]): Promise<ModuleReloadResult[]> {
    logger.info(`[ModuleReloader] 批量热更新 ${modules.length} 个模块`);
    ...
    for (const module of modules) {
      ...
      if (!result.success) {
        logger.error(`[ModuleReloader] 模块 ${module.modulePath} 热更新失败，停止批量更新`);
        break;
      }
    }

    const successCount = results.filter(r => r.success).length;
    logger.info(`[ModuleReloader] 批量热更新完成: ${successCount}/${modules.length} 成功`);
  ...
  async checkCircularDependency(modulePath: string): Promise<boolean> {
    // TODO: 实现循环依赖检查
    // 可以使用madge等工具
    logger.info(`[ModuleReloader] TODO: 检查 ${modulePath} 的循环依赖`);
    return false;
  }
}
