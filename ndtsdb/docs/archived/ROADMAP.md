...
> **产品愿景**: 成为时序智能引擎 (TSI Engine) 标杆，超越 DuckDB/ClickHouse  
> **目标**: 金融级实时决策基础设施，云边端一体化  
> **愿景文档**: `docs/VISION.md`
...
### 近期交付
- ✅ Virtual Market Generator (量化策略数据生成)
- ✅ StreamingIndicators 扩展 (RSI/MACD/BB)
- ✅ ndtsdb-cli 产品规划 (QuickJS + libndtsdb)
- ✅ 核心技术需求验证计划
...
```
Phase 1 (当前-Q2 2026)    Phase 2 (Q3 2026)        Phase 3 (Q4 2026)       Phase 4 (2027)
     │                          │                       │                      │
     ▼                          ▼                       ▼                      ▼
┌──────────────┐          ┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│ 单机卓越      │    ───►  │ 边缘就绪      │ ───► │ 分布式       │ ───► │ 智能引擎      │
│             │          │             │      │             │      │             │
│ • 金融语义SQL │          │ • ndtsdb-cli │      │ • 分布式协调器 │      │ • AI 训练    │
│ • 验证测试   │          │ • WASM支持   │      │ • 自动分片    │      │ • GPU加速    │
│ • 性能优化   │          │ • 边缘云同步  │      │ • 负载均衡    │      │ • 自优化查询  │
│             │          │ • 嵌入式AI   │      │             │      │             │
└──────────────┘          └──────────────┘      └──────────────┘      └──────────────┘
```
...
**目标**: 单机性能达到标称值，金融场景验证通过

| 任务 | 状态 | 交付物 |
|------|------|--------|
| 金融语义 SQL 扩展 | 🔄 进行中 | `OHLCV()`, `ASOF JOIN`, `REPLAY()` |
| 核心技术验证 | 🔄 进行中 | 高并发/压缩率/回放稳定性测试报告 |
| 指标库完善 | ✅ 已完成 | RSI/MACD/BB 流式计算 |
| 虚拟市场数据 | ✅ 已完成 | Virtual Market Generator |

**验收标准**:
- 压缩率: Gorilla 达到 70-90%（真实数据）
- 回放性能: 8.9M ticks/sec @ 3000 品种
- SQL 功能: 覆盖 90% 金融查询场景
...
| 任务 | 状态 | 负责 | 交付物 |
|------|------|------|--------|
| ndtsdb-cli | ⏳ 待启动 | 1号/006 | QuickJS + libndtsdb 单二进制 |
...
| 联邦学习 | ⏳ 愿景 | 分布式模型训练 |
| 向量化执行 | ⏳ 愿景 | SIMD 全面优化 |
| GPU 加速 | ⏳ 愿景 | CUDA/Metal 支持 |
| 自优化查询 | ⏳ 愿景 | AI-based 查询优化器 |

---

## 🎯 任务优先级建议 (2026-02-18)

**综合评分文档**: `docs/ROADMAP-SCORED.md`

基于业务价值、技术紧迫性、实现复杂度、资源依赖四个维度评分，TOP 建议如下：

### 🔴 P0 - 立即执行 (本周)

| 排名 | 任务 | 综合分 | 关键理由 |
|------|------|--------|----------|
| 1 | **压缩率实测验证** | 8.5 | Gorilla 标称70-90%，实际可能仅20-30%，阻塞生产决策 |
| 2 | **金融语义 SQL** | 7.9 | 差异化核心，OHLCV/REPLAY 是金融场景刚需 |
| 3 | **回放稳定性测试** | 7.6 | 9号实盘依赖，3000品种长时间运行验证 |

### 🟡 P1 - 本周内

| 任务 | 综合分 | 说明 |
|------|--------|------|
| 高并发压力测试 | 7.1 | 验证生产环境承载能力 |
| 错误处理规范化 | 6.7 | 技术债，可交给007顺便做 |

### 🟢 P2 - 本月内

| 任务 | 综合分 | 决策条件 |
|------|--------|----------|
| zstd/lz4 压缩集成 | 6.1 | **如压缩率实测<50%，提升为P0** |
| ndtsdb-cli | 5.8 | **如1号评估可行，提升为P1** |
| mmap/Zero-Copy 深度验证 | 6.2 | 大文件场景验证 |

### 💤 按需 / ⚪ 未来

- 压缩算法 C 实现 (4.9) - 仅当性能瓶颈时
- 分布式系列 (4.1-3.6) - Phase 3 再看
- 智能引擎 (愿景) - Phase 4

**本周行动计划**:
- 02-18: 压缩率测试启动
- 02-19: 金融语义 SQL 设计 + 回放测试启动  
- 02-20: 高并发压力测试
- 02-23: Phase 1 验证报告
...
**背景**: 以下需求是开发阶段提出的核心能力，需系统性验证实际表现。

| 维度 | 核心需求 | 当前状态 | 待验证项 | 验证方式 | 优先级 |
|------|----------|----------|----------|----------|--------|
| **并发性** | 读写高并发 | Append-only + WAL 顺序写 | 多线程写入安全性、读写混合负载、连接池管理 | 压力测试：100/1000/3000 品种并发 | P1 |
| **存储效率** | 压缩存储比 | Gorilla/Delta/RLE 已实现 | 真实 K-line 压缩率、压缩/解压 CPU 开销 | 虚拟市场数据实测 vs 官方数据 | P1 |
| **SQL 接口** | 完整 SQL 支持 | SELECT/JOIN/GROUP BY/窗口函数 | 复杂 JOIN 性能、SQL 解析器稳定性、参数化查询 | 功能测试 + 性能基准 | P2 |
| **实时回放** | 多时序流回放 | MmapMergeStream + SmartPrefetcher | 3000+ 品种稳定性、内存增长曲线、回放延迟抖动 | 全市场回放实测 | P1 |
| **底层优化** | mmap + Zero-Copy | MmappedColumnarTable 已实现 | 随机读 vs 顺序读性能、内存映射上限、零拷贝实现细节 | 大文件测试 (1GB/10GB/100GB) | P2 |

#### Phase 1: 基础功能验证（1-2 天）
- [ ] 虚拟市场数据写入 + 压缩率实测
- [ ] 单表 SQL 查询功能覆盖测试
- [ ] 单品种回放性能基准

#### Phase 2: 压力测试（2-3 天）
- [ ] 多品种并发写入（100/1000/3000 品种）
- [ ] 读写混合负载测试
- [ ] 大文件 mmap 性能（1GB/10GB/100GB）

#### Phase 3: 边界测试（1 天）
- [ ] 内存占用长时间运行测试
- [ ] 崩溃恢复测试（kill -9 后数据完整性）
- [ ] 极限压缩比测试（不同数据分布）

**验收标准**:
- 压缩率达到官方标称值（Gorilla 70-90%）
- 回放性能达到标称值（8.9M ticks/sec）
- 多品种并发无内存泄漏
- 崩溃后数据可恢复
