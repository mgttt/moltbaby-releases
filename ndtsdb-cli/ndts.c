...
NDTSDB* ndtsdb_open_snapshot(const char* path, uint64_t snapshot_size) {
    fprintf(stderr, "[DEBUG] ndtsdb_open_snapshot: path=%s\n", path);
    ...
    if (db->is_dir) {
        ...
        while ((ent = readdir(dir)) != NULL) {
            ...
            while (!feof(f)) {
                chunk_num++;
                fprintf(stderr, "[DEBUG] Reading chunk %d, pos=%ld\n", chunk_num, ftell(f));
                // 检查 snapshot 限制
                ...
                if (fread(&row_count, 4, 1, f) != 1) {
                    fprintf(stderr, "[DEBUG] Breaking: fread failed, feof=%d\n", feof(f));
                    break;
                }
                if (row_count == 0) {
                    fprintf(stderr, "[DEBUG] Breaking: row_count=0\n");
                    break;
                }
                ...
                for (uint32_t i = 0; i < row_count; i++) {
                    ...
                    fprintf(stderr, "[DEBUG] Adding row %u/%u: %s/%s ts=%lld\n", i, row_count, sym, itv, (long long)timestamps[i]);
